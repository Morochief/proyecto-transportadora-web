...
def listar_remitentes():
    ...
    remitentes = query.order_by(Remitente.id.desc()).paginate(page=page, per_page=per_page)
    return jsonify({
        "items": [
            {
                "id": r.id,
...
def modificar_remitente(id):
    ...
    remitente.tipo_documento = data.get('tipo_documento', remitente.tipo_documento)
    remitente.numero_documento = data.get('numero_documento', remitente.numero_documento)
